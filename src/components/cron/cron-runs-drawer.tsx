/**
 * Cron Runs Drawer
 * Side panel showing job execution history with lastRun fallback
 */
...
import type { CronJobRun, CronJobLastRun } from '@/types/cron';
  ...
  return (
    <div className="p-3 rounded-lg border space-y-2">
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
        ...
        {run.retryCount != null && run.retryCount > 0 && <span>{t('runs.retries')}: {run.retryCount}</span>}
      </div>
      {run.error && (
        <div className="p-2 rounded bg-red-50 dark:bg-red-900/20 text-xs text-red-600 dark:text-red-400">{run.error}</div>
      )}
    </div>
  );
}

export function CronRunsDrawer({ jobId, jobName, lastRun, onClose }: CronRunsDrawerProps) {
  const { t } = useTranslation('cron');
  const { runs, runsLoading, fetchRuns } = useCronStore();
  const isOpen = jobId !== null;

  useEffect(() => { if (jobId) fetchRuns(jobId); }, [jobId, fetchRuns]);
  ...
  // Fallback: convert job's lastRun to CronJobRun format when cron.runs returns empty
      ...
      <div className="fixed inset-y-0 right-0 z-50 w-full max-w-md bg-background border-l shadow-xl flex flex-col animate-in slide-in-from-right duration-200">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {runsLoading ? (
            ...
            <>
              {runs.length === 0 && lastRun && (
                <p className="text-xs text-muted-foreground italic mb-2">{t('runs.lastRunFallback')}</p>
              )}
              {displayRuns.map((run) => <RunEntry key={run.id} run={run} />)}
            </>
          )}
        </div>
      </div>
