...
async function start() {
  try {
    ...
    const broadcastUsb = (data: Record<string, unknown>) => {
      if (!wss) return;
    ...
    try {
      ...
      if (envToken && envToken.trim().length > 0) {
        ...
        if (!isValid) {
          ...
          const ensureDnsRecord = async (recordName: string, dnsSubdomainName: string) => {
          ...
          let dashboardDnsSubdomain: string;
          if (envDomain === zoneName) {
          ...
          let terminalDnsSubdomain: string;
          if (envDomain === zoneName) {
