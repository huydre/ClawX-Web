...
const getClawHubCliPath = (): string => {
...
const runClawHubCommand = (args: string[]): Promise<string> => {
...
const parseSkillList = (output: string): Array<{ slug: string; name: string; version: string; description: string }> => {
  ...
  return lines.map(line => {
    ...
    return null;
  }).filter((s): s is { slug: string; name: string; version: string; description: string } => s !== null);
};
