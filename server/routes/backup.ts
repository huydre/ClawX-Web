...
router.get('/export', async (_req, res) => {
  try {
    const dirs: string[] = [];
    if (existsSync(OPENCLAW_DIR)) dirs.push('.openclaw');
    if (existsSync(CLAWX_DIR)) dirs.push('.clawx');
    ...
    await pipeline(
      tar.create(
        { gzip: true, cwd: homedir(), portable: true },
        dirs,
      ),
      res,
    );
...
router.post('/restore', upload.single('backup'), async (req, res) => {
  ...
  try {
    ...
    await tar.list({
      ...
      onReadEntry: (entry: any) => { entries.push(entry.path); },
    });
    ...
    // Cleanup on error
    try { if (uploadedFile?.path) unlinkSync(uploadedFile.path); } catch { /* ignore */ }
...
router.get('/info', async (_req, res) => {
  try {
    const info: { openclaw: boolean; clawx: boolean } = {
