...
export interface UsbDevice {
  deviceId: string;      // e.g. "sda1"
  label: string;         // volume label or "USB Drive"
  mountPath: string;     // e.g. "/media/techla/MYUSB"
...
function categorizeFile(filename: string): UsbFile['category'] {
  const ext = extname(filename).toLowerCase();
  for (const [category, extensions] of Object.entries(FILE_CATEGORIES)) {
    if (extensions.includes(ext)) {
      return category as UsbFile['category'];
    }
  }
  return 'other';
}
...
export class UsbMonitor extends EventEmitter {
  ...
  getFiles(deviceId: string, subPath?: string): UsbFile[] {
  ...
  readFile(deviceId: string, filePath: string, maxBytes = 100 * 1024): { content: string; truncated: boolean } | null {
    const device = this.devices.get(deviceId);
    if (!device || device.status !== 'ready') return null;

    const absPath = join(device.mountPath, filePath);

    // Security: ensure path is within mount
    if (!absPath.startsWith(device.mountPath)) return null;
  ...
  async eject(deviceId: string): Promise<boolean> {
    const device = this.devices.get(deviceId);
  ...
  async copyToWorkspace(deviceId: string, filePaths: string[], destDir: string): Promise<{ copied: string[]; errors: string[] }> {
    const device = this.devices.get(deviceId);
    const copied: string[] = [];
    const errors: string[] = [];
  ...
  private detectUsbPartitions(): Array<{ name: string; size: string; label: string; mountpoint: string | null }> {
    try {
      const result = execSync(
        'lsblk --json --output NAME,SIZE,LABEL,MOUNTPOINT,TRAN,TYPE 2>/dev/null',
        { encoding: 'utf-8', timeout: 5000 },
      );

      const parsed = JSON.parse(result);
      const partitions: Array<{ name: string; size: string; label: string; mountpoint: string | null }> = [];
  ...
  private async handleNewDevice(part: { name: string; size: string; label: string; mountpoint: string | null }): Promise<void> {
  ...
  private scanDevice(device: UsbDevice): ScanSummary {
    const categories: Record<string, number> = { documents: 0, code: 0, data: 0, media: 0, other: 0 };
    let totalFiles = 0;
    let totalSize = 0;

    const scan = (dir: string, depth: number): void => {
      if (totalFiles >= MAX_FILE_SCAN || depth > 20) return;

      try {
        const entries = readdirSync(dir);
        for (const entry of entries) {
          if (totalFiles >= MAX_FILE_SCAN) break;
          if (entry.startsWith('.')) continue;
