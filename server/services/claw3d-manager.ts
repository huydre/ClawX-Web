...
type Claw3dState = 'stopped' | 'cloning' | 'installing' | 'starting' | 'running' | 'error';
...
function generateEnv(gatewayPort: number): string {
  return [
...
class Claw3dManager extends EventEmitter {
  ...
  private state: Claw3dState = 'stopped';
  private error: string | null = null;
  private port = DEFAULT_PORT;

  private setState(state: Claw3dState, error?: string) {
    this.state = state;
  ...
  async setup(gatewayPort = 18789): Promise<void> {
  ...
  async start(gatewayPort = 18789): Promise<void> {
  ...
  async restart(gatewayPort = 18789): Promise<void> {
    await this.stop();
  ...
  private runCommand(cmd: string, args: string[], cwd?: string): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      child.on('close', (code) => {
        if (code === 0) resolve();
        else reject(new Error(`${cmd} ${args.join(' ')} exited with code ${code}: ${stderr.slice(-200)}`));
      });
