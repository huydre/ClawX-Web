...
function readPending(): Record<string, { requestId: string }> {
  try {
...
export function startAutoPairing() {
  ...
  gatewayManager.on('notification', async (event: string, payload: any) => {
  ...
  gatewayManager.on('stateChange', async (state: string) => {
    if (state === 'connected') {
