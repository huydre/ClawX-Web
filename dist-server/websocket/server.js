                ...
                if (method === 'agent' || method === 'chat') {
                    ...
                    if (eventState === 'final') {
                        ...
                        const hasToolUse = Array.isArray(content) &&
                            content.some((b) => b.type === 'tool_use' || b.type === 'toolCall');
                        if (hasToolUse) {
                        ...
                        if (usage && (usage.input_tokens || usage.prompt_tokens || usage.total_tokens)) {
                            ...
                            const model = String(message?.model || params?.data?.model || 'unknown');
                            const provider = String(params?.data?.provider || message?.provider || model.split('/')[0] || 'unknown');
