...
import { saveOAuthTokenToOpenClaw, getOAuthTokenFromOpenClaw, setOpenClawDefaultModel, } from '../utils/openclaw-sync.js';
const router = Router();
// ── Codex OAuth Config ─────────────────────────────────────────────────────
const CODEX_CONFIG = {
 ...
 */
function startCallbackServer(codeVerifier, state) {
...
router.get('/codex/start', async (_req, res) => {
    try {
        ...
        startCallbackServer(codeVerifier, state)
            ...
            // Set default model
            setOpenClawDefaultModel('openai-codex', 'gpt-5.4');
