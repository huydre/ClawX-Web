...
router.post('/save', (req, res) => {
    try {
        const { type, config, accountId } = req.body;
        ...
        if (PLUGIN_CHANNELS.includes(type)) {
            ...
            if (type === 'openzalo') {
                ...
                if (config.profile && typeof config.profile === 'string' && config.profile.trim()) {
                    zaloConfig.profile = config.profile.trim();
                }
                ...
                if (config.zcaBinary && typeof config.zcaBinary === 'string' && config.zcaBinary.trim()) {
                    zaloConfig.zcaBinary = config.zcaBinary.trim();
                }
        ...
        if (type === 'telegram') {
            ...
            if (allowFrom && typeof allowFrom === 'string') {
                const users = allowFrom.split(',').map(u => u.trim()).filter(u => u.length > 0);
                if (users.length > 0) {
                    transformedConfig.allowFrom = users;
                }
            }
            // Only add allowFrom: ["*"] when dmPolicy is explicitly "open"
            const dmPolicy = transformedConfig.dmPolicy;
            if (dmPolicy === 'open') {
                let af = transformedConfig.allowFrom;
                if (!af || !Array.isArray(af))
                    af = ['*'];
                else if (!af.includes('*'))
                    af.push('*');
        ...
        if (type === 'discord') {
            ...
            if (guildId && typeof guildId === 'string' && guildId.trim()) {
                const guildConfig = { users: ['*'], requireMention: true };
        ...
        if (type === 'discord') {
            ...
            if (guilds) {
                ...
                if (guildIds.length > 0) {
                    ...
                    const channels = guilds[guildIds[0]]?.channels;
                    if (channels) {
