...
async function proxyFetch(path, init) {
    if (!PROXY_URL)
        throw new Error('Composio proxy not configured');
    const headers = {
...
export async function getConnection(connectionId) {
    ...
    if (res.status === 404)
        return null;
    if (!res.ok)
        throw new Error(`Proxy get connection failed: ${res.status}`);
    return (await res.json());
}
export async function deleteConnection(connectionId) {
    if (!isProxyConfigured())
        return; // mock — nothing remote to delete
    const res = await proxyFetch(`/api/connections/${encodeURIComponent(connectionId)}`, {
        method: 'DELETE',
    });
...
export async function proxyStatus() {
    ...
    try {
        ...
        if (!res.ok)
            return { reachable: false, configured: false, error: `HTTP ${res.status}` };
